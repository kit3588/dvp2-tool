...
#define DEV_SN1			"DSGP50200000017"
#define DEV_SN2			"DSGQ12700000000"
	...
	********************/
	...
	{   
		...
		{        
			...
			/*  这里演示修改序设备列号为“DSGB20300000001”的IP */
			if(strcmp(info[i].OriginalSerialNumber , DEV_SN1)
			&& strcmp(info[i].OriginalSerialNumber , DEV_SN2))
			{
				continue;
			}
			...
			{
				printf("%s->%d:[%s] DeviceMac:%s, DeviceIP:%s, DeviceNetMask:%s, DeviceGateway:%s, EthIP:%s\n", 
					__FILE__, __LINE__, info[i].FriendlyName, GetIP.DeviceMac, GetIP.DeviceIP, GetIP.DeviceNetMask, GetIP.DeviceGateway,GetIP.EthIP);
				...
				{
					/* 指定设备新的IP */
					snprintf(SetIP.DeviceIP, sizeof(SetIP.DeviceIP), "192.168.10.100");
				
				}
				else if(!strcmp(info[i].OriginalSerialNumber , DEV_SN2))
				{
					/* 指定设备新的IP */
					snprintf(SetIP.DeviceIP, sizeof(SetIP.DeviceIP), "192.168.2.100");
